			...
			if(c_chackCode==null){
				...
				if(username==null || "".equals(username)){
					...
					if(ud.isUserExist(username)) {
						request.getSession().setAttribute("error", "<script>alert(\"用户名已经存在\")</script>");
						mes.put("addErrorMes", "用户名已经存在");
						out.print(WebUtil.mapToJson(mes));
						out.print("<script>alert(\"用户名已经存在\")</script>");
						out.flush();
						return;
					}
					...
					user.setU_pic("images/user/1576901406554.png");
